...
import { Archive, EditIcon, LucideSparkles } from "lucide-react";
    ...
    <aside className="bg-black text-[#DFDBCE] hidden overflow-hidden md:block h-screen w-[260px]">
      <div className="flex flex-col h-full pb-4">
        <ScrollArea className="px-3 flex-1 relative pt-3">
          <Link href={"#"} className="sticky z-50 top-0">
            <div className="bg-black hover:bg-[#202123] px-3 py-2 flex justify-between items-center p-1 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="grow overflow-hidden text-ellipsis whitespace-nowrap text-sm font-semibold">
                  New chat
                </span>
              </div>
              <div>
          ...
          <div className="relative mt-5 h-auto w-[230px]">
            <div>
            ...
            <ol>
              {new Array(2).fill("").map((chat) => (
                <Chat
                  chatId="15b85a37-79d1-4526-90c9-57159bbbcc63"
                  label="Express API with TypeScript"
                />
              ))}
            </ol>
          </div>
          ...
          <Link href={"#"}>
            <div className="min-h-[44px] flex gap-2 items-center p-2 hover:bg-[#202123] rounded-md">
              <div className="w-7 h-7 bg-black border p-1 border-gray-800 rounded-full flex justify-center items-center">
                <LucideSparkles fill="#fff" />
              </div>
              <div className="flex flex-col">
                <span className="font-semibold text-sm">Upgrade plan</span>
                <span className="text-xs">Get GPT-4, DALL·E, and more</span>
              </div>
            </div>
          </Link>
...
const Chat = (props: ChatProps) => {
  const { chatId, label } = props;
  return (
    <li className="relative h-auto">
      <div className="group rounded-lg active:opacity-90 hover:bg-[#202123]">
        <a
          href={`/chat/${chatId}`}
          className="flex text-sm font-medium items-center gap-2 p-2"
        >
